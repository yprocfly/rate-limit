...
class RedisRateLimit:
    ...
    def get_limit_config(self, key_name):
        """
        ...
        if LimitConfig.use_redis:
            ...
            limit_config = json.loads(conn.hget(LimitConfig.service, key_name) or '{}')
    ...
    def attempt_get_token(self, key_name, limit_config=None):
        """
        ...
        try:
            if not limit_config:
                limit_config = self.get_limit_config(key_name)
            ...
            conn = get_redis_connection()
            result = conn.execute_command('CL.THROTTLE', key_name, total_quota, limit_quota, limit_second, once_quota)
...
if __name__ == '__main__':
    ...
    print(redis_rate_limit.attempt_get_token('key_name'))
